...
%let syscc=0;
options nonotes nosource nosource2 ps=max noquotelenmax;
...
put 'put ''    </h1>'';';
put 'put '' '';';
put 'put ''    <h4 class="secHide">JSON Format</h4>'';';
...
put 'put ''    <h4 class="secHide">SAS Format</h4>'';';
...
put 'url=''http://localhost/SASLogon/oauth/clients/consul?callback=false&serviceId=app'';';
...
put 'url=''http://localhost/SASLogon/oauth/clients'';';
...
put 'put ''                let access_token_macro = chunkString(res.tokeninfo[0].ACCESS_TOKEN, 220).join(''''%trim(\r\n)'''');'';';
put 'put ''                let refresh_token_macro = chunkString(res.tokeninfo[0].REFRESH_TOKEN, 220).join(''''%trim(\r\n)'''');'';';
